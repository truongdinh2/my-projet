...
const FormLearner = () => {
  ...
  const onFinish = async (params) => {
    // setWho(params.who)
    await router.push('/nguoi-hoc/dang-nhap')
    setAuth('nguoihoc')
  }
  const bank = (
  ...
  return (
    <div className={styles.page}>
      ...
      <Row>
        <Row style={{ width: '60%' }} justify="center">
          <Col span={20}>
            <Row justify="center" gutter={[14, 14]} className={styles.btn_ng_hoc_ng_than}>
              <Col offset={5}>
                <Button style={{ marginRight: '5px' }} onClick={() => setRole('nguoi hoc')}>
                  {auth === 'nguoihoc' ? 'Người thân' : 'Người học'}
                </Button>
              </Col>
            </Row>
            <Form
            ...
            >
              ...
              {(isRelation && role === 'nguoi hoc' && auth !== 'nguoihoc') && <Relation />}
              {(isRelation && auth !== 'nguoihoc') && <User />}
        ...
        <Row>
          <Col style={{ position: 'fixed', top: "26%", zIndex: "-1" }}>
            <NoteForm />
          </Col>
        </Row>
      </Row>
